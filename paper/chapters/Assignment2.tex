...
In order to make a quantitative comparison, two things are needed. First of all, the quadrilaterals have to be found fully automatical (as described in \sectionref{sec:unsupervised_painting_detection}). The second thing is the solution created in assignment 1 (see \chapterref{chap:assignment1}), which is the presumably perfect paintings. The first part for finding these presumably perfect painting, is retrieving these from the database. 

...
def draw_quadrilaterals(graph, quadrilaterals, point_size=15, width=2, color="red"):
    ...
    for q in quadrilaterals:
        TopLineId = graph.DrawLine(
            (q.TLPoint.x, q.TLPoint.y), (q.TRPoint.x, q.TRPoint.y), color=color, width=width)
        RightLineId = graph.DrawLine(
            (q.TRPoint.x, q.TRPoint.y), (q.BRPoint.x, q.BRPoint.y), color=color, width=width)
        BottomLineId = graph.DrawLine(
            (q.BRPoint.x, q.BRPoint.y), (q.BLPoint.x, q.BLPoint.y), color=color, width=width)
        LeftLineId = graph.DrawLine(
            (q.BLPoint.x, q.BLPoint.y), (q.TLPoint.x, q.TLPoint.y), color=color, width=width)

        TLPointId = graph.DrawRectangle((q.TLPoint.x - point_shift, q.TLPoint.y - point_shift), (q.TLPoint.x +
                                                                                                 point_shift, q.TLPoint.y + point_shift), fill_color=color, line_color=None, line_width=None)
        TRPointId = graph.DrawRectangle((q.TRPoint.x - point_shift, q.TRPoint.y - point_shift), (q.TRPoint.x +
                                                                                                 point_shift, q.TRPoint.y + point_shift), fill_color=color, line_color=None, line_width=None)
        BLPointId = graph.DrawRectangle((q.BLPoint.x - point_shift, q.BLPoint.y - point_shift), (q.BLPoint.x +
                                                                                                 point_shift, q.BLPoint.y + point_shift), fill_color=color, line_color=None, line_width=None)
        BRPointId = graph.DrawRectangle((q.BRPoint.x - point_shift, q.BRPoint.y - point_shift), (q.BRPoint.x +
                                                                                                 point_shift, q.BRPoint.y + point_shift), fill_color=color, line_color=None, line_width=None)

        quadrilateralFigures.append(QuadrilateralFigure(
            TLPointId, TRPointId, BRPointId, BLPointId, TopLineId, RightLineId, BottomLineId, LeftLineId))
